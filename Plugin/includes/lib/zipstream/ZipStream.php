...
class ZipStream {
	...
	*/
	 ...
	 * @var array
	 */
	protected $zipStreamMeta;
	
	
	
	protected $bandwith_limit = 0;
	...
	public function addDirectoryFromStream($directory_name) {
		$this->addFileFromStreamStart($directory_name);
		$this->addFileFromStreamChunk('');
		$this->addFileFromStreamEnd();
	}

	public function addFileFromStreamStart($name, $opt = array()) {
		$block_size = 1048576; // process in 1 megabyte chunks
		$algo       = 'crc32b';
		$meth       = 0x00;
		$genb       = 0x08;
	...
	public function addFileFromStreamChunk($data) {
		$len = $this->zipStreamMeta['len'];
		$zlen = $this->zipStreamMeta['zlen'];
		$hash_ctx = $this->zipStreamMeta['hash_ctx'];
		...
		$this->zipStreamMeta['hash_ctx'] = $hash_ctx;
		$this->zipStreamMeta['len'] = $len;
		$this->zipStreamMeta['zlen'] = $zlen;
	}

	public function addFileFromStreamEnd(){
		$meth = 0x00;
		$genb = 0x08;
		$name = $this->zipStreamMeta['name'];
		$hash_ctx = $this->zipStreamMeta['hash_ctx'];
		$len = $this->zipStreamMeta['len'];
		$zlen = $this->zipStreamMeta['zlen'];
		$opt = $this->zipStreamMeta['opt'];
		$num_bytes_written = $this->zipStreamMeta['num_bytes_written'];
	...
	public function set_bandwith_limit($bytes) {
		$this->bandwith_limit = $bytes;
	}
	...
	protected function send($str) {
		...
		// bandwith limit section
		$bytes_sent += strlen($str);
		if ($this->bandwith_limit && $bytes_sent >= $this->bandwith_limit) {
			$bytes_sent = 0;
			sleep(1);
		}
	}
